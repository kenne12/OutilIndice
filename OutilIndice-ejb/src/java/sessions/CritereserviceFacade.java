...
public class CritereserviceFacade extends AbstractFacade<Critereservice> implements CritereserviceFacadeLocal {
    ...
    public List<Critereservice> findByIdService(long idService) {
        Query query = this.em.createQuery("SELECT c FROM Critereservice c WHERE c.service.idservice=:idService ORDER BY c.service.code");
    ...
    public Critereservice findByIdServiceIdCritere(long idService, int idCritere) {
        Query query = this.em.createQuery("SELECT c FROM Critereservice c WHERE c.service.idservice=:idService AND c.critere.idcritere=:idCritere");
        query.setParameter("idService", (idService)).setParameter("idCritere", (idCritere));
        List<Critereservice> list = query.getResultList();
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
