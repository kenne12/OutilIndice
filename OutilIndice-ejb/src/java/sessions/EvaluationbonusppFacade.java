...
public class EvaluationbonusppFacade extends AbstractFacade<Evaluationbonuspp> implements EvaluationbonusppFacadeLocal {
    ...
    public Evaluationbonuspp findByIdPersonnel(int idPeriode, int idSousPeriode, long idPersonnel, int idCritere) {
        Query query = em.createQuery("SELECT e FROM Evaluationbonuspp e WHERE e.idperiode.idperiode=:idPeriode AND e.idsousperiode.idsousperiode=:idSousPeriode AND e.idpersonnel.idpersonnel=:idPersonnel AND e.idcritere.idcritere=:idCritere");
