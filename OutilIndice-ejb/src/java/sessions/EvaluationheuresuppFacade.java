...
public class EvaluationheuresuppFacade extends AbstractFacade<Evaluationheuresupp> implements EvaluationheuresuppFacadeLocal {
    ...
    public Evaluationheuresupp findByIdPersonnel(int idPeriode, int idSousPeriode, long idPersonnel, int idCritere) {
        Query query = em.createQuery("SELECT e FROM Evaluationheuresupp e WHERE e.idnote.idperiode.idperiode=:idPeriode AND e.idnote.idsousperiode.idsousperiode=:idSousPeriode AND e.idnote.idpersonnel.idpersonnel=:idPersonnel AND e.idcritere.idcritere=:idCritere");
