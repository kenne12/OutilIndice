...
public class PrimeQntifDeptCtrl extends AbstractPrimeQntifDeptCtrl {
    ...
    public void updateFiltre() {
        ...
        if (service.getIdservice() != null && service.getIdservice() > 0) {
            if (sousperiode.getIdsousperiode() != null && sousperiode.getIdsousperiode() > 0) {
                List<Cible> list = cibleFacadeLocal.findByIdSousPeriode(SessionMBean.getStructure().getIdstructure(), periode.getIdperiode(), sousperiode.getIdsousperiode(), 4);
    ...
    public void removeIndicator(Cible c) {
        if (c.getIdcible() != 0l) {
            ...
            cibles.remove(c);
            listCibles = cibleFacadeLocal.findByIdSousPeriode(SessionMBean.getStructure().getIdstructure(), periode.getIdperiode(), sousperiode.getIdsousperiode(), 4);
